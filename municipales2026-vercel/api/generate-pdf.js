...
// ── Recherche presse par département ──────────────────────────────────────
async function fetchPresseParDept(depts, apiKey) {
const presseData = {};
const deptsActifs = depts.filter(d =>
(d.victoires_1t||0)+(d.victoires_2t||0)+(d.qualifies_2t||0) > 0
).slice(0, 6); // Limiter à 6 depts pour ne pas dépasser le timeout

for (const dept of deptsActifs) {
const deptNom = DEPT_NOMS[dept.dept] || dept.departement || dept.dept;
try {
const prompt = `Recherche les principaux titres et faits de presse locale concernant les elections municipales 2026 dans le departement ${deptNom} (${dept.dept}) en Nouvelle-Aquitaine. Synthetise en 2-3 phrases courtes en ASCII uniquement les elements marquants rapportes par la presse locale (resultats notables, surprises, enjeux). Si aucune information disponible, reponds simplement "Aucune information presse disponible."`;
...
```
  ...
  if (response.ok) {
    ...
    if (text && text.length > 20) {
      // Nettoyer le texte : supprimer accents et caractères spéciaux
...
const quals  = (crs||[]).filter(c => c.statut&&c.statut.includes(‘2nd Tour’)&&!c.statut.includes(‘Victoire’));
...
`Le groupe PS/PP et apparentes totalise ${blocPSPP?(blocPSPP.victoires_1t||0)+(blocPSPP.victoires_2t||0):totalElus} victoires et ${blocPSPP?(blocPSPP.qualifies_2t||0):stats.ball||0} qualifications au 2nd tour.`,
`La photographie reste evolutive selon les resultats du 2nd tour a venir.`,
],
faits_marquants: [
`Gironde (33) : ${(depts.find(d=>d.dept==='33')?.victoires_1t||0)} victoires au 1er tour.`,
...
bloc:b.bloc, engages:b.engages||0,
elus:(b.victoires_1t||0)+(b.victoires_2t||0),
qualifies:b.qualifies_2t||0,
defaites:(b.defaites_1t||0)+(b.defaites_2t||0),
})),
points_attention: [
stats.ball>0 ? `${stats.ball} conseillers sont encore en lice pour le 2nd tour.` : null,
...
// ── Appel Claude (page 1 — analyse textuelle) ─────────────────────────────
async function callAnthropic(data) {
const apiKey = process.env.ANTHROPIC_API_KEY;
if (!apiKey) throw new Error(‘ANTHROPIC_API_KEY manquante’);
const elus  = (data.crs||[]).filter(c=>c.statut===‘Victoire 1er Tour’||c.statut===‘Victoire 2nd Tour’).map(c=>({nom:c.nom,commune:c.commune,dept:c.dept,score:c.s1}));
...
Redige UNIQUEMENT les sections textuelles (resume, faits, blocs, points attention) de la note d’analyse.
Les listes d’elus par departement sont gerees separement — ne les inclus PAS dans ta reponse.
...
- ASCII uniquement (pas d’accents, pas de caracteres speciaux Unicode)
- Tonalite institutionnelle sobre
- Citer les noms des elus les plus emblematiques dans les faits marquants
- Ne jamais inventer de resultat
...
}`; const response = await fetch(ANTHROPIC_API_URL, { method:'POST', headers:{'content-type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01'}, body: JSON.stringify({model:MODEL,max_tokens:2000,temperature:0.2,messages:[{role:'user',content:prompt}]}), }); if (!response.ok) throw new Error(`Anthropic ${response.status}`);
const json = await response.json();
const text = pickText(json.content);
...
const newPage = () => { page=pdfDoc.addPage([W,H]); drawHeader(); };
...
[‘Bloc politique’,‘Engages’,‘Elus’,‘Qualifies’,‘Defaites’].forEach((h,i)=>{
...
vals.forEach((v,i)=>{ page.drawText(v,{x:colXs[i]+4,y:y-rowH+8,size:i===0?8:8.5,font:i===0?fR:fB,color:colors[i]}); });
y-=rowH;
});
y-=8;
...
// ── PAGE(S) 2+ — Résultats par département ────────────────────────────────
newPage();
...
const presseText = presseData[deptCode];
...
if(d.elus.length>0){
  ...
  for(const nom of d.elus){
    ensureY(13);
    page.drawText('v',{x:M+12,y,size:8,font:fB,color:C.vert});
    page.drawText(nom,{x:M+24,y,size:8.5,font:fR,color:C.noir});
    y-=13;
  }
}
...
if(d.qualifies.length>0){
  ...
  for(const nom of d.qualifies){
    ensureY(13);
    page.drawText('>',{x:M+12,y,size:8,font:fB,color:C.orange});
    page.drawText(nom,{x:M+24,y,size:8.5,font:fR,color:C.noir});
    y-=13;
  }
}
...
if(presseText){
  ...
  page.drawText('Eclairage presse :',{x:M+16,y,size:7.5,font:fB,color:C.bleu});
  y-=13;
...
// Construire elus_par_dept avec groupe politique
const elus  = crs.filter(c=>c.statut==='Victoire 1er Tour'||c.statut==='Victoire 2nd Tour');
const quals = crs.filter(c=>c.statut&&c.statut.includes('2nd Tour')&&!c.statut.includes('Victoire'));
...
const deptData = deptsSorted
  .filter(d=>(elusByDept[d.dept]||[]).length>0||(qualsByDept[d.dept]||[]).length>0)
  .map(d=>({
...
// Lancer analyse Claude et recherche presse en parallèle
let analysis;
let presseData = {};

const [analysisResult, presseResult] = await Promise.allSettled([
  callAnthropic(data),
  apiKey ? fetchPresseParDept(deptsSorted, apiKey) : Promise.resolve({}),
]);

analysis = analysisResult.status === 'fulfilled' ? analysisResult.value : buildFallbackAnalysis(data);
presseData = presseResult.status === 'fulfilled' ? presseResult.value : {};
