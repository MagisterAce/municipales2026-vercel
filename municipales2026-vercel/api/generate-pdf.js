...
function buildFallbackAnalysis(data) {
  ...
  return {
    ...
    // IMPORTANT: elus_par_dept construit directement depuis crs — pas depuis Claude
    elus_par_dept: deptsSorted.map(d=>({
...
// ── Appel Claude (page 1 uniquement — analyse textuelle) ──────────────────
...
async function buildPdf(analysis, deptData, meta={}) {
  ...
  const drawHeader = () => {
    ...
    // Date de génération à droite (plus "mis à jour")
  ...
  for (const d of deptData) {
    ...
    const nbLines = (d.elus.length>0 ? 2+d.elus.length : 0) + (d.qualifies.length>0 ? 2+d.qualifies.length : 0);
    const needed = 38 + nbLines*13 + 14;
    ensureY(needed);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Construire elus_par_dept DIRECTEMENT depuis les données CR (pas depuis Claude)
    const crs = data.crs||[];
    ...
    // Claude génère uniquement le texte analytique (page 1)
    let analysis;
